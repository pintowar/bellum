...
object DataFixtures {
    ...
    fun generateEstimator(project: Project, mtx: List<List<Long>>): Map<Employee, Map<Task, Duration>> {
        return project.allEmployees().zip(mtx).associate { (employee, row) ->
            val tasks = project.allTasks().zip(row).associate { (task, duration) ->
    ...
    val smallTimeEstimator = object : TimeEstimator {
        private val cache = generateEstimator(sampleProjectSmall, listOf(
            listOf(10L, 20, 30, 40, 50), // Employee 0
            listOf(15L, 25, 35, 45, 55), // Employee 1
            listOf(12L, 22, 32, 42, 52)  // Employee 2
        ))

        override fun estimate(employeeSkills: Array<Int>, taskSkills: Array<Int>): Duration =
            TODO("Not yet implemented")

        override fun estimate(employee: Employee, task: Task): Duration {
            return cache.getValue(employee).getValue(task)
        }
    }
}