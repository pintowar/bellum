...
abstract class TimeEstimator {
    ...
    open fun estimate(
        ...
        val skillValidation = validateSkillSets(employeeSkills, taskSkills)
        return when (skillValidation) {
            is Either.Left -> Either.Left(skillValidation.value)
            is Either.Right -> skillsEstimation(employeeSkills, taskSkills)
        }
    }
